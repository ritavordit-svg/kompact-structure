        ...
        MongoDbCrudOpration("global", obj, "findOne")
            .then((response) => {
                ...
                if (response && response.AssignCompany && response.AssignCompany.length > 0) {
                    ...
                    MongoDbCrudOpration("global", cObj, "findOne")
                        .then((company) => {
                            if (company) {
                                res.send({
                                    status: true,
...
exports.getUserById = async (req, res) => {
    try {
        ...
        const users = await MongoDbCrudOpration(SCHEMA_TYPE.GLOBAL, userObj, "findOne");
        users.Employee_profileImageURL = `${process.env.APIURL}storage/USER_PROFILES/${users.Employee_profileImage}`;
...
exports.getUserByQuey = async (req, res) => {
    exports
        .getUserByQueyFun(req.body.query, req.body.companyId, req.body.fetchFromCache)
        .then((data) => {
            res.json(data);
        })
        .catch((error) => {
...
exports.getUserByQueyFun = async (query = {}, companyId = "", fetchFromCache = false, preAggregate = false) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const users = await MongoDbCrudOpration(SCHEMA_TYPE.GLOBAL, userObj, "aggregate");
            ...
            users.forEach((user) => {
                user.Employee_profileImageURL = buildUserAvatarUrl(user.Employee_profileImage);

                myCache.set(`UserData:${user._id}`, JSON.stringify(user), 604800);
            });
