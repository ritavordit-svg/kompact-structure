...
exports.importReactionEmojis = (companyName) => {
    const dataArray = [
        {
            name: "Love",
            value: "LOVE",
            shortCode: "/-love",
            image: "reactions/love.svg",
            statusImage: "reactions/love.svg",
            position: 1,
            isDeleted: false
        },
        {
            ...
            statusImage: "reactions/haha.svg",
            position: 2,
            isDeleted: false
        },
        {
        ...
        },
        {
            ...
            value: "ANGRY",
            shortCode: "/-angry",
            image: "reactions/angry.svg",
            statusImage: "reactions/angry.svg",
            position: 5,
            isDeleted: false
        },
        {
            ...
            statusImage: "reactions/like.svg",
            position: 6, // 👈 LIKE để cuối
            isDeleted: false
        }
    ];
...
// IMPORT STICKER PACK + STICKER
const fs = require('fs');
const path = require('path');
exports.importStickerData = async (companyId) => {
    try {
        const stickerRoot = path.join(__dirname, "..", "public", "images", "sticker");

        let config = {};
        const configPath = path.join(stickerRoot, "sticker_config.json");
        if (fs.existsSync(configPath)) {
            config = JSON.parse(fs.readFileSync(configPath, "utf8"));
        }

        await MongoDbCrudOpration(companyId, { type: "sticker_packs", data: [{}] }, "deleteMany");
        await MongoDbCrudOpration(companyId, { type: "stickers", data: [{}] }, "deleteMany");

        // Scan folder
        const packFolders = fs.readdirSync(stickerRoot).filter(folder =>
            fs.statSync(path.join(stickerRoot, folder)).isDirectory()
        );

        for (const folder of packFolders) {

            const originalName = config[folder]?.name || folder;   // tên tiếng Việt
            const description = config[folder]?.description || `Sticker pack: ${originalName}`;

            const slug = folder.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/\s+/g, "_").toLowerCase();

            const srcDir = path.join(stickerRoot, folder);
            const destDir = path.join(__dirname, "..", "storage", companyId.toString(), "stickers", slug);
            ...
            // Tạo pack
            const newPack = await MongoDbCrudOpration(companyId, {
                type: "sticker_packs",
                data: {
                    name: originalName,       // tên tiếng Việt
                    slug,
                    description,
                    isActive: true,
                    coverImage: `stickers/${slug}/${files[0]}`,
                    createdAt: new Date(),
                    updatedAt: new Date(),
                }
            }, "save");
            ...
            console.log("✔ Imported pack:", originalName);
        }

        console.log("✔ Hoàn tất import sticker!");
    } catch (err) {
        console.error(err);
        throw err;
    }
...
exports.importReactionEmojis = async (companyId) => {
    try {
        const sourceDir = path.join(__dirname, "..", "public", "images", "reactions");
        const destDir = path.join(__dirname, "..", "storage", companyId.toString(), "reactions");

        fs.mkdirSync(destDir, { recursive: true });

        const reactions = [
            { name: "Love", value: "LOVE", file: "love.svg" },
            { name: "Haha", value: "HAHA", file: "haha.svg" },
            { name: "Wow",  value: "WOW",  file: "wow.svg" },
            { name: "Sad",  value: "SAD",  file: "sad.svg" },
            { name: "Angry",  value: "ANGRY",  file: "angry.svg" },
            { name: "Like", value: "LIKE", file: "like.svg" },
        ];

        // copy ảnh
        for (const r of reactions) {
            fs.copyFileSync(
                path.join(sourceDir, r.file),
                path.join(destDir, r.file)
            );
        }
        ...
        const dataObj = {
            ...
            settings: reactions.map(r => ({
                name: r.name,
                value: r.value,
                image: `reactions/${r.file}`,
                statusImage: `reactions/${r.file}`,
                isDeleted: false
            }))
        };
        ...
        await MongoDbCrudOpration(companyId, obj, "findOneAndUpdate");
        console.log("✔ Reaction emojis imported");

    } catch (err) {
        console.error("❌ importReactionEmojis error:", err);
        throw err;
    }
};
